...
void fill_ellipse(Uint32 *pixels, int pitch, int tex_w, int tex_h, SDL_Color color) {
    int cx = tex_w / 2;
    int cy = tex_h / 2;
    float rx = tex_w / 2.0f;
    float ry = tex_h / 2.0f;

           // Zbuduj kolor w formacie ARGB8888
    Uint32 argb_color = (color.a << 24) | (color.r << 16) | (color.g << 8) | (color.b);
    ...
    for (int y = 0; y < tex_h; ++y) {
        ...
        for (int x = 0; x < tex_w; ++x) {
            ...
            if (distance <= 1.0f) {
                row[x] = argb_color;  // Wypełnij kolorem
            } else {
                row[x] = 0x00000000;  // Przezroczysty
            }
        }
    }
}
        ...
        for (unsigned int x = 0; x < width.value; x++)
        {
            ...
            switch(f)
            {
                case Form_solid:
                {
                 ...
                 //   pixels[y * width.value + x % (1 + rand() % width.value)] = (distance <= 1.0f) ? c : 0;
                    break;
                }
                case Form_liquid:                    
                    ...
                    pixels[y * width.value + x ] = (distance <= offset) ? c : 0x20000000;
                //    pixels[y * width.value + width.value - x -1] = (distance <= 1.0f) ? c : 0;
                    break;
                case Form_gas:
                    c |= 0xffff00 | get_base()->color.b; ; c=0xffffffff;

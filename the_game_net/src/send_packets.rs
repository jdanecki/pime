...
pub extern "C" fn send_packet_action_on_object(client: &mut NetClient, a: i32, oid: usize) {
    let mut buf = vec![core::PACKET_PLAYER_ACTION_ON_OBJECT];
...
#[no_mangle]
pub extern "C" fn send_packet_server_action_on_object(client: &mut NetClient, a: i32, oid: usize) {
    let mut buf = vec![core::PACKET_SERVER_ACTION_ON_OBJECT];
    buf.extend_from_slice(&a.to_le_bytes());
    buf.extend_from_slice(&oid.to_le_bytes());
    client.send(&buf);
}
